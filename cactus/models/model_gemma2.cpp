...
Gemma2Model::Gemma2Model() : Model() {}

Gemma2Model::Gemma2Model(const Config& config) : Model(config) {
    weight_nodes_.layers.resize(config.num_layers);
    weight_nodes_.attn_softcap_val = config.attn_logit_softcap;
    weight_nodes_.final_softcap_val = config.final_logit_softcap;
    weight_nodes_.rope_freq = config.rope_theta;
}

void Gemma2Model::post_init() {
    kv_cache_.set_window_size(0, 0); 
}

void Gemma2Model::load_weights_to_graph(CactusGraph* gb) {
...
size_t Gemma2Model::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    bool is_global_attention = ((layer_idx + 1) % 2) == 0;

    if (config_.rope_theta > 0) {
        q_proj_4d = gb->rope(q_proj_4d, weight_nodes_.rope_freq, position_offset);
        k_proj_4d = gb->rope(k_proj_4d, weight_nodes_.rope_freq, position_offset);
    }
    ...
    size_t window_size = is_global_attention ? 0 : 4096;
    ...
    if (use_cache && !kv_cache_.is_empty()) {
        attn_output_4d = gb->attention_int8_hybrid(
            ...
            kv_cache_.current_seq_len, num_kv_heads, config_.attention_head_dim, window_size,
            weight_nodes_.attn_softcap_val
        );
    } else {
        attn_output_4d = gb->attention(q_proj_4d, k_proj_4d, v_proj_4d, attention_scale_,
                                        position_offset, window_size, backend, weight_nodes_.attn_softcap_val);
    }
...
size_t Gemma2Model::build_mlp(CactusGraph* gb, size_t input, uint32_t layer_idx,
...
size_t Gemma2Model::build_transformer_block(CactusGraph* gb, size_t hidden, uint32_t layer_idx,
                                         ComputeBackend backend, bool use_cache, size_t position_offset) {
    const auto& layer = weight_nodes_.layers[layer_idx];
...
size_t Gemma2Model::forward(const std::vector<uint32_t>& tokens, bool use_cache) {
